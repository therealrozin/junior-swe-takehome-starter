...
- Type definitions matching the backend Product model
...
The API client in `src/api/products.ts` connects to:
```
http://localhost:3000/api/products
```

If the backend is on a different URL, update the `API_BASE` constant in `src/api/products.ts`.
...
├── App.tsx               # Main app component with TODO comments
├── App.css               # Styling
├── types/
│   └── Product.ts        # TypeScript types
├── components/
│   └── ProductCard.tsx   # Complete component example
└── api/
